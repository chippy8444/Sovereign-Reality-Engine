...
app.get('/api/status', (req, res) => res.json({ ok: true, app: 'Resolution AI / Sovereign Reality Engine', version: '0.3.1-blue-no-cache', port: PORT, provider: (process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY) ? 'Claude / Anthropic' : 'Local evidence fallback', dataRoot: DATA_ROOT, settings: getSettings() }));
...
app.post('/api/chat', async (req, res) => { const question = safeText(req.body?.message || req.body?.question).trim(); if (!question) return res.status(400).json({ error: 'message required' }); const trace = [{ step: 'classify', status: 'ok' }]; const evidence = searchGraph(question, getSettings().maxGraphNodes || 8); trace.push({ step: 'graph', status: 'ok', count: evidence.length }); const draft = await draftAnswer(question, evidence); trace.push({ step: 'model', status: (process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY) ? 'claude_or_fallback' : 'local_fallback' }); const verification = refine(question, draft, evidence); trace.push({ step: 'ra_refinery', status: verification.status, grade: verification.grade }); const aud = audit({ type: 'chat', questionHash: sha256(question), verification, evidenceIds: verification.evidenceIds }); res.json({ answer: draft, verification, trace, audit: { id: aud.hash, prevHash: aud.prevHash }, evidence }); });
